$(document).ready(function() {
	...
	$local.tablaMantenimiento = $local.$tablaMantenimiento.DataTable({
		"ajax" : {
			"url" : $variableUtil.root + "mantenimiento/oferta?accion=buscarTodos",
			"dataSrc" : ""
		},
		"language" : {
	...
	$local.$registrarMantenimiento.on("click", function() {
		...
		$.ajax({
			...
			success : function(oferta) {	
				...
				var row = $local.tablaMantenimiento.row.add(oferta).draw();
				row.show().draw(false);
	...
	$local.$tablaMantenimiento.children("tbody").on("click", ".actualizar", function() {
		...
		var oferta = $local.tablaMantenimiento.row($local.$filaSeleccionada).data();
		$local.idOferta = oferta.nIdOferta;
		$funcionUtil.llenarFormulario(oferta, $formMantenimiento);
	...
	$local.$actualizarMantenimiento.on("click", function() {
		...
		$.ajax({
			...
			success : function(oferta) {
				...
				var row = $local.tablaMantenimiento.row($local.$filaSeleccionada).data(oferta).draw();
				row.show().draw(false);
				$(row.node()).animateHighlight();
				$local.$modalMantenimiento.PopupWindow("close");
			},
